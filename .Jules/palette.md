...
### 2026-07-01 — About the Author heading hierarchy fix
- **Target:** `_layouts/post.html`
- **Finding:** "About the Author" heading is an `<h3>`, causing a skipped heading level since the previous sibling section is an `<h2>` ("Ready to connect?").
- **Action:** Upgraded the `<h3>` to an `<h2>` while adding the Bootstrap `h3` utility class to preserve its visual styling.
